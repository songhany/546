...
app.use(async (req, res, next) => {  //next is a special parameter that's going to either call the next middleware that we have. Or without any other middleware function, it's going to go on to the route that we request
  totalRequests++;
  console.log(`There have been ${totalRequests} requests made to the server`);
  next();  //after next() execute, you will go to the next. If we have another middleware, you Either go to the next middleware function that's in the stack. OR if there is no other middleware function, you'll continue on to the route that we request, in this case, you don't have other middleware function and without next(), you cannot hit the routes, it will always pending
});

// 2. One which will count the number of requests that have been made to the current path
const pathsAccessed = {};  //store all the paths that were accessed in this object
...
let admin = false;
app.use('/admin', async (req, res, next) => {  //❤ this middleware function only apply to /admin routers. If you don't add path like /admin, this middleware function just apply to every route on server
  if (!admin) {
    console.log("I'm in the admin middleware");
    return res.status(403).json({ error: '403: Forbidden' });  //kill the response
  } else {
    console.log("I'm in the admin middleware");
    next();
  }
...
app.use(async (req, res, next) => {
  ...
  if (totalRequests % 5 === 0) {
    ...
    res.cookie('lastAccessed', '', { expires: anHourAgo });  // expire the cookie after a specific time
    res.clearCookie('lastAccessed');  // clears the cookie specified by name. http://expressjs.com/en/api.html#res.clearCookie
  ...
  res.cookie('lastAccessed', now.toString(), { expires: expiresAt });  // add cookie. 1st field for cookie name, 2nd for cookie value, 3nd for expires
