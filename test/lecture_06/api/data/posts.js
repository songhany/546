...
const users = require('./users');  //need async method of users
...
const exportedMethods = {
  ...
  async addPost(title, body, tags, posterId) {
    ...
    if (!Array.isArray(tags)) {  //❤ tag don't supply, we initialize it to an empty array
      tags = [];
    }

    const postCollection = await posts();

    const userThatPosted = await users.getUserById(posterId);  //users' method
  ...
  async removePost(id) {
    ...
    await users.removePostFromUser(post.poster.id, id);  //'post.poster.id' is userId, 'id' is postId
    return true;
  },
  ...
  async renameTag(oldTag, newTag) {
    ...
    const postCollection = await posts();
    await postCollection.updateMany(findDocuments, firstUpdate);  //add newTag to tags array
    await postCollection.updateMany(findDocuments, secondUpdate);  //remove oldTag from tags array
