...
async function runSetup() {
  ...
  try {
    ...
    await db.collection('advancedMovies').drop();  //drop 'advancedMovies' collection
  } catch (e) {
  ...
  const listOfMovies = [];  //after we created all movie, we insert all movie into list

  
  const inception = makeDoc('Inception', 4.5, 2015, 'Christopher Nolan');
  inception.cast.push(  //array of cast
    'Leonardo DiCaprio',
    'Ellen Page',
    'Ken Watanabe',
  ...
  // array of reviews add three review below
  addReview(
    inception,
    'Really Good',
