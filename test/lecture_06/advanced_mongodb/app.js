...
async function main() {
  ...
  const startUp = await startUpDocs.runSetup();  // run db.collection('advancedMovies').drop() firstly; then initialize movie data 
  ...
  const addCastNoDup = await advMongo.addCastMemberIfNotExists(1, 'Leonardo DiCaprio');  // $addToSet
  console.log(addCastNoDup);

  const addCastDup = await advMongo.addCastMemberAllowDuplicates(1, 'Leonardo DiCaprio');  // $push
